...
## 11.1 Function Calling 失败场景 + 兜底方案
核心原则：**永远不要信任 LLM 输出的 tool_call，所有校验、重试、熔断全部在代码层兜底，不能交给大模型自己处理**。
## 一、模型侧：输出不符合工具调用规范（最常见）

### 场景 1：模型输出纯文本，没有生成 tool_calls（本该调用工具，但是直接回答文字）

现象：`last_message.tool_calls = None`，业务中断。
触发：

- prompt 不够清晰、few-shot（少样本提示） 不足；
- `tool_choice="auto"`时模型自作主张直接回答；
- 上下文过长，模型忘记需要调用工具。
兜底：

1. 想强制调用工具：使用`tool_choice="any/required"`，强制模型必须输出 tool_call；
2. 加代码判断：如果预期必须调用工具但`tool_calls`为空，构造错误消息丢回 LLM 重试；
3. 最大重试次数（例如最多 2~3 轮重试，防止死循环）。

### 场景 2：tool_call 格式非法（JSON 解析失败、字段缺失）

现象：模型输出乱的文本，不是合法 JSON；缺少`name`/`args`/`id`字段。
触发：长上下文、复杂 prompt、模型幻觉。
兜底：

1. 结构化输出约束（`with_structured_output`），让模型严格按 Schema 输出；
2. try-catch 捕获 JSON 解析异常，把**解析错误信息**放回消息历史，让 LLM 修正；
3. 重试熔断，超过次数直接走人工 HITL 中断（interrupt，就是你邮件 agent 里的人工审核）。

### 场景 3：幻觉，调用不存在的工具名称

现象：`tool_call["name"]`不在你`tools_by_name`字典里面，代码取工具直接抛 KeyError。
兜底：

1. 代码层判断：`if tool_name not in tools_by_name`，捕获这个错误，把`不存在该工具`的观察结果返回给 LLM；
2. Prompt 里明确写死可用工具列表，禁止调用其他函数；
3. 工具白名单硬编码校验。

### 场景 4：参数 args 幻觉，字段不存在、类型错误、参数缺失

现象：函数需要`email:str`，模型传`email:null`；或者传了不存在字段，少必填参数。

> 
> NL2SQL 场景：模型编造不存在的表名、字段名放到 args 的 SQL 里。
> 兜底：

1. 使用 Pydantic 对工具入参做**强类型校验**；
2. 捕获参数校验异常，把错误信息作为 tool 返回结果回传给 LLM；
3. 业务层校验：比如 SQL 执行前，校验表、字段是否存在；邮件工具校验收件人格式。

## 二、工具执行阶段失败（tool_node 内部执行报错）

### 场景 5：工具执行抛异常（网络超时、数据库报错、权限不足）

例子：

- `schedule_meeting`：日历服务 API 超时；
- NL2SQL 的`run_sql`：SQL 语法错误、主键冲突、查询超时。
现象：`tool.invoke()`抛出 Exception。
兜底：

1. try/except 包裹工具执行代码，**不要直接让整个 agent 崩溃**；捕获异常，把异常文本包装成`tool`角色消息返回给 LLM；
2. 区分可重试错误 / 不可重试：
   - 可重试：网络超时、临时连接失败 → 有限次数重试；
   - 不可重试：权限拒绝、参数非法 → 直接停止或者进入人工审核 HITL；
3. 增加超时控制：每个工具调用设置 timeout，防止卡死。

### 场景 6：工具执行成功，但返回结果为空 / 无效结果

现象：函数没报错，但是返回空列表、空字符串，数据和用户问题无关（比如查订单，返回空）。
触发：LLM 生成的查询条件不对。
兜底：

1. 在 tool 返回后增加结果校验节点；
2. 把「查询返回空」作为观察结果丢回 LLM，让模型重新调整参数再次调用工具；
3. 达到最大重试次数后，触发人工确认（HITL），询问用户是否修改查询条件。

## 三、多工具并行调用场景（parallel tool call，一次生成多个 tool_call）

### 场景 7：部分 tool 成功，部分 tool 调用失败

现象：一次返回多个 tool_call，其中 1 个执行报错，其他正常。
兜底：

1. 分开捕获每个 tool 的异常，**不要一个失败全部作废**；成功的保留结果，失败的带上错误信息返回；
2. 传给 LLM 完整的观测（哪些成功、哪些失败 + 原因），由模型决定是否重试失败的工具。

## 四、状态与链路异常（LangGraph 特有，你现在这套架构重点）

### 场景 8：tool_call_id 不匹配

现象：工具返回的`tool_call_id`和 AI 消息里的 id 对不上，大模型无法把工具结果和原始调用绑定，上下文错乱。
兜底：

1. 代码强制把 tool_call 里原始 id 填入 tool 返回消息，**不要让 LLM 自己生成 id**；
2. 增加 id 一致性校验。

### 场景 9：死循环（无限重试）

现象：LLM 反复生成错误 tool_call，循环一直在 llm_call → tool_node 来回跑，永不结束。
触发：模型一直无法修正错误，每次执行都报错。
兜底（非常重要）：

1. 在`state`里维护**重试计数器**，每重试一次 + 1；
2. 设置最大轮数阈值（例如最多 3~5 轮工具调用）；
3. 到达阈值，不再重试，分支跳转 HITL 人工中断，或者直接结束并返回错误给用户。

## 五、业务权限 & 安全类失败（高危，必须硬编码拦截，不能交给 LLM）

### 场景 10：模型生成危险调用（删数据、越权查询）

例子：NL2SQL 场景，LLM 写出`DROP TABLE`、`DELETE`语句。
兜底：

1. 独立安全校验节点，黑名单拦截高危操作；
2. 数据库使用只读账号执行查询，从底层禁止写操作；
3. 敏感工具（发邮件、排会议）强制走 HITL 人工审核（就是你 interrupt_handler），**执行前必须人确认**。

## 六、网络 / LLM 服务侧异常

### 场景 11：调用 LLM 接口超时、限流、服务 500 报错

现象：`llm.invoke()`抛出异常，无法生成 tool_call。
兜底：
...
1. **第一层：Prompt + Schema（预防）**
   - 清晰工具描述、Few-shot 样例；用 Pydantic 定义工具入参，让 bind_tools 拿到严格 schema。
2. **第二层：代码校验 + 异常捕获（拦截，核心）**
   - 校验工具名、参数类型、业务合法性；try-except 捕获所有异常；
   - 状态维护计数器，限制最大工具调用轮次，防止死循环。
3. **第三层：HITL 人工兜底（最后防线）**
当重试耗尽、模型持续出错、操作高危时，进入 interrupt，交给人判断：
   - 修改参数后继续执行
   - 直接拒绝本次工具调用
   - 终止整个 Agent 流程

# 放到你现有 LangGraph 架构里的改造点

1. 在`tool_node`内部加 try except，捕获工具执行异常；
2. 在 state 增加`retry_count`字段，每次进入 llm_call+1；
3. 在`should_continue`路由判断：如果`retry_count >= max_retry`，路由跳转到`interrupt_handler`人工节点；
4. 高危工具`write_email / schedule_meeting`强制人工审核，普通只读工具直接放行。